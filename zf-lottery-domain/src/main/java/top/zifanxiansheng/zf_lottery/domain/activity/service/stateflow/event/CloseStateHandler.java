...
@ActivityState(value = Constants.ActivityStateEnum.CLOSE)
@Service
public class CloseStateHandler extends AbstractStateHandler {
    ...
    public Result arraignment(Long activityId, Constants.ActivityStateEnum state) {
        return Result.buildFailure("关闭状态不提审");
    }
    ...
    public Result pass(Long activityId, Constants.ActivityStateEnum state) {
        return Result.buildFailure("关闭状态不能通过");
    }

    @Override
    public Result reject(Long activityId, Constants.ActivityStateEnum state) {
        return Result.buildFailure("关闭状态不能拒绝");
    ...
    public Result open(Long activityId, Constants.ActivityStateEnum state) {
        boolean success = activityRepository.alterActivityStatus(activityId, state, Constants.ActivityStateEnum.OPEN);
