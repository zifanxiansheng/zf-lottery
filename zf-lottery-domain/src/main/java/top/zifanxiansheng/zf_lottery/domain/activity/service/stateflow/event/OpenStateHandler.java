 ...
 **/
@ActivityState(value = Constants.ActivityStateEnum.OPEN)
@Service
public class OpenStateHandler extends AbstractStateHandler {
    ...
    public Result pass(Long activityId, Constants.ActivityStateEnum state) {
        return Result.buildFailure("开启状态不能通过");
    }

    @Override
    public Result reject(Long activityId, Constants.ActivityStateEnum state) {
        return Result.buildFailure("开启状态不能拒绝");
    ...
    public Result open(Long activityId, Constants.ActivityStateEnum state) {
        return Result.buildFailure("不能重复开启");
